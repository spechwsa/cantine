    ...
    public String index( Model model ) {
        ...
        try {
            serviceCantine.enregisterPresence( new Enseignant( "a", "Françoise", "MULLER" ),
                    new Eleve( "a", "François", "THOMAS" ) );
        } catch ( EnseignantNonAutentifieException e ) {
            // TODO Auto-generated catch block
            e.printStackTrace();
        }

        Map<Eleve, Boolean> eleveCantineMap = eleveRepository.getAll().stream().collect(
                Collectors.toMap(
                        eleve -> {
                            return eleve;
                        },
        ...
        // On stocke les élèvesCantine dans le Model sous l'attribut
        // "EleveCantineMap"
